...
@push('scripts')
    ...
    <script>
        ...
        const deleteAuction = (id) => {
            try {
                let url = '{{ route('admin.key_feature.destroy', ':id') }}';
    ...
    <script>
        // Show Confirmation Alert Before Sponsored Status Change
        function showSponsoredChangeAlert(event, checkboxElement, id) {
            event.preventDefault();
            let newStatus = checkboxElement.checked ? 1 : 0;

            Swal.fire({
                title: 'Are you sure?',
                text: 'You want to update the sponsored status to ' + (newStatus === 1 ? 'Enabled' : 'Disabled') + '?',
                icon: 'info',
                showCancelButton: true,
                ...
                updateSponsoredStatus(id, newStatus);
            });
        }

        // AJAX Call to Update Sponsored Status
        function updateSponsoredStatus(id, newStatus) {
            var url = '{{ route('admin.product.status', ['id' => ':id']) }}';
            $.ajax({
                type: "POST",
                headers: {
